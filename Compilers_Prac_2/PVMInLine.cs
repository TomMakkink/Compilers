...
//            if (tracing) results.WriteLine();
          ...
          case PVM.mul:           // integer multiplication
            tos = mem[cpu.sp++];
            long tmp = Convert.ToInt64(tos)*Convert.ToInt64(mem[cpu.sp]);
            if (tmp > maxInt || tmp < (-1*maxInt)) ps = badMem;
            else
              mem[cpu.sp] = Convert.ToInt32(tmp);
            break;
          ...
          case PVM.prnc:          // character output
            results.WriteLine((char)mem[cpu.sp++]);
            break;
          ...
          case PVM.islet:         // isLetter
            if (Char.IsLetter((char)mem[cpu.sp])) 
              mem[cpu.sp] = 1;
            else  
              mem[cpu.sp] = 0;
            break;
